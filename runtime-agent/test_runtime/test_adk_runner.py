...
def _make_adk_event_with_tool_result(
    action: str, rationale: str, executed: bool = False
):
...
class TestADKRunnerWiring:
    ...
    @pytest.mark.asyncio
    async def test_endpoint_extracts_no_op_decision(
    ...
    @pytest.mark.asyncio
    async def test_endpoint_extracts_block_decision(
        self, sample_event_dict: dict, make_pubsub_body: callable
    ):
    ...
    @pytest.mark.asyncio
    async def test_endpoint_fallback_on_adk_failure(
        self, sample_event_dict: dict, make_pubsub_body: callable
    ):
        ...
        assert body["decision"] == "NO_OP"
        assert (
            "fallback" in body.get("status", "accepted").lower()
            or body["decision"] == "NO_OP"
        )

    @pytest.mark.asyncio
    async def test_endpoint_fallback_on_no_tool_result(
        self, sample_event_dict: dict, make_pubsub_body: callable
    ):
        """POST /events/runtime uses NO_OP if ADK produces no tool calls."""
        text_event = _make_adk_event_text(
            "I analyzed the event but made no conclusion."
        )
    ...
    @pytest.mark.asyncio
    async def test_guard_blocked_decision_preserved(
        self, sample_event_dict: dict, make_pubsub_body: callable
    ):
        ...
        guard_event.content.parts[0].function_response.response[
            "guard_reason"
        ] = "opa_violation"
    ...
    @pytest.mark.asyncio
    async def test_policy_refs_enriched_for_block(
        self, sample_event_dict: dict, make_pubsub_body: callable
    ):
